...
void SignalHandler::onStartCommand()
{
	...
	Core::debugLog.logDebug(SOURCE_LOCATION_ARGS, L"Signals blocked in the main thread");
...
void SignalHandler::onStopCommand()
{
	...
	Core::debugLog.logDebug(SOURCE_LOCATION_ARGS, L"Signal handler thread has been stopped");
	...
	Core::debugLog.logDebug(SOURCE_LOCATION_ARGS, L"Signals unblocked in the main thread");
	setState<IdlingState>();
...
bool SignalHandler::onSignal(int signo)
{
	...
	switch (signo) {
		...
		default:
			...
			Core::warningLog.logMessage(oss.str());
			return true;
	}
}
...
void SignalHandler::SignalHandlerThread::run()
{
	try {
		...
		while (true) {
			...
			if (!signalHandlerState.equals<AbstractSubsystem::RunningState>()) {
				if (signalHandlerState.equals<AbstractSubsystem::StoppingState>()) {
					...
					// TODO
					Core::warningLog.logDebug(SOURCE_LOCATION_ARGS, L"Signal handler unexpected state '" +
							signalHandlerState.value().name() + L"' detected before inspecting pending signals");
				}
			}
		...
		Core::errorLog.logException(e, SOURCE_LOCATION_ARGS, L"Executing signal handler thread error. Stopping subsystem.");
	} catch (...) {
		Core::errorLog.logDebug(SOURCE_LOCATION_ARGS, L"Executing signal handler thread unknown error. Stopping subsystem.");
	}
