...
SystemCallError::SystemCallError(SystemCallError::Function function, int errnum, SOURCE_LOCATION_ARGS_DECLARATION) :
	AbstractError(composeMessage(function, errnum), SOURCE_LOCATION_ARGS_PASSTHRU),
	_function(function),
	_errnum(errnum)
{}
	
std::wstring SystemCallError::getFunctionName(Function function)
{
	switch (function) {
		// pthread functions
		case PThreadCreate:
...
std::wstring SystemCallError::composeMessage(Function function, int errnum)
{
	std::wostringstream msg;
	msg << getFunctionName(function) << L" system call error: (" << errnum << L") " << strerror(errnum);
	return msg.str();
}

AbstractError * SystemCallError::clone() const
{
