...
HttpMessageStreamWriter_NEW::HttpMessageStreamWriter_NEW(AbstractIODevice& device) :
	_device(device),
	_header(),
	_transmissionStarted(false),
	_chunkedHeaderComposed(false),
...
HttpMessageStreamWriter_NEW::~HttpMessageStreamWriter_NEW()
{}

void HttpMessageStreamWriter_NEW::reset()
{
	_header.clear();
	_transmissionStarted = false;
	_chunkedHeaderComposed = false;
...
void HttpMessageStreamWriter_NEW::setHeaderField(const std::string &fieldName, const std::string &fieldValue, bool replaceIfExists)
{
	if (replaceIfExists) {
		std::pair<Header::iterator, Header::iterator> range = _header.equal_range(fieldName);
		for (Header::iterator i = range.first; i != range.second; ++i) {
...
bool HttpMessageStreamWriter_NEW::headerContains(const std::string &fieldName, const std::string &fieldValue) const
{
	std::pair<Header::const_iterator, Header::const_iterator> range = _header.equal_range(fieldName);
	for (Header::const_iterator i = range.first; i != range.second; ++i) {
		if (i->second.first == fieldValue) {
...
std::list<std::string> HttpMessageStreamWriter_NEW::headerValues(const std::string &fieldName) const
{
	std::list<std::string> result;
	std::pair<Header::const_iterator, Header::const_iterator> range = _header.equal_range(fieldName);
	for (Header::const_iterator i = range.first; i != range.second; ++i) {
...
Http::Params HttpMessageStreamWriter_NEW::header() const
{
	Http::Params result;
	for (Header::const_iterator i = _header.begin(); i != _header.end(); ++i) {
		result.insert(Http::Params::value_type(i->first, i->second.first));
...
void HttpMessageStreamWriter_NEW::removeHeaderField(const std::string &fieldName)
{
	std::pair<Header::iterator, Header::iterator> range = _header.equal_range(fieldName);
	for (Header::iterator i = range.first; i != range.second; ++i) {
		if (i->second.second) {
...
bool HttpMessageStreamWriter_NEW::writeChunk(const char * buffer, size_t bufferSize, const Timestamp& limit, size_t * bytesWrittenToDevice)
{
	if (bytesWrittenToDevice) {
		*bytesWrittenToDevice = 0;
	}
	...
	_sendBufferBytesSent = 0;
	while (true) {
		size_t bytesSent = _device.write(_sendBuffer.data() + _sendBufferBytesSent, _sendBuffer.size() - _sendBufferBytesSent, limit.leftTo());
		if (bytesSent > 0) {
...
bool HttpMessageStreamWriter_NEW::writeOnce(const char * buffer, size_t bufferSize, const Timestamp& limit, size_t * bytesWrittenToDevice)
{
/*	if (bytesWrittenToDevice) {
		*bytesWrittenToDevice = 0;
	}
...
bool HttpMessageStreamWriter_NEW::finalize(const Timestamp& limit, size_t * bytesWrittenToDevice)
{
/*	if (bytesWrittenToDevice) {
		*bytesWrittenToDevice = 0;
	}
...
bool HttpMessageStreamWriter_NEW::flush(const Timestamp& limit, size_t * bytesWrittenToDevice)
{
/*	if (bytesWrittenToDevice) {
		*bytesWrittenToDevice = 0;
	}
...
std::string HttpMessageStreamWriter_NEW::composeHeader()
{
	std::string result;
	for (Header::iterator i = _header.begin(); i != _header.end(); ++i) {
		if (i->second.second) {
