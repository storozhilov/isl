...
//! Base class for HTTP-message stream writers
class HttpMessageStreamWriter_NEW
{
public:
	//! Constructor
	/*!
	  \param device I/O-device for data to send
	*/
	HttpMessageStreamWriter_NEW(AbstractIODevice& device);
	//! Destructor
	virtual ~HttpMessageStreamWriter_NEW();
	//! Returns a reference to the I/O device
	inline AbstractIODevice& device() const
	{
		return _device;
	...
	*/
	...
	/*!
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	inline bool writeChunk(const std::string& str, const Timestamp& limit, size_t * bytesWrittenToDevice = 0)
	{
		return writeChunk(str.data(), str.length(), limit, bytesWrittenToDevice);
	}
	//! Sends chunked encoded NULL-terminated string
	/*!
	  If string is empty this method does nothing and returns true.
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	inline bool writeChunk(const char * str, const Timestamp& limit, size_t * bytesWrittenToDevice = 0)
	{
		return writeChunk(str, strlen(str), limit, bytesWrittenToDevice);
	}
	//! Sends chunked encoded buffer
	/*!
	  If bufferSize <= 0 this method does nothing and returns true.
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	bool writeChunk(const char * buffer, size_t bufferSize, const Timestamp& limit, size_t * bytesWrittenToDevice = 0);
	//! Sends unencoded STL string and finalizes HTTP-message
	/*!
	  \param str STL string to send
	  \param limit Data transfer limit timestamp
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	inline bool writeOnce(const std::string& str, const Timestamp& limit, size_t * bytesWrittenToDevice = 0)
	{
		return writeOnce(str.data(), str.length(), limit, bytesWrittenToDevice);
	}
	//! Sends unencoded NULL-terminated string and finalizes HTTP-message
	/*!
	  \param str NULL-terminated string to send
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	inline bool writeOnce(const char * str, const Timestamp& limit, size_t * bytesWrittenToDevice = 0)
	{
		return writeOnce(str, strlen(str), limit, bytesWrittenToDevice);
	}
	//! Sends unencoded buffer and finalizes HTTP-message
	/*!
	  \param buffer Buffer to send
	  ...
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	bool writeOnce(const char * buffer, size_t bufferSize, const Timestamp& limit, size_t * bytesWrittenToDevice = 0);
	//! Sends bodyless HTTP-message
	/*!
	  \param limit Data transfer limit timestamp
	  \param bytesWrittenToDevice Pointer to memory location where number of bytes have been sent to the device is to be put
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	inline bool writeBodyless(const Timestamp& limit, size_t * bytesWrittenToDevice = 0)
	{
		return writeOnce(0, 0, limit, bytesWrittenToDevice);
	}
	//! Sends last (empty) chunk and the trailer of the HTTP-message
	/*!
	  \param limit Data transfer limit timestamp
	...
	*/
	bool finalize(const Timestamp& limit, size_t * bytesWrittenToDevice = 0);
	//! Sends all unsent data
	/*!
	  \param limit Data transfer limit timestamp
	  \param bytesWrittenToDevice Pointer to memory location where number of bytes have been sent to the device is to be put
	  \return TRUE if all data has been send to peer or FALSE if the I/O timeout has been expired and flush() call is needed to complete an operation
	*/
	bool flush(const Timestamp& limit, size_t * bytesWrittenToDevice = 0);
protected:
	//! HTTP-message first line composition method
	virtual std::string composeFirstLine() const = 0;
private:
	HttpMessageStreamWriter_NEW();
